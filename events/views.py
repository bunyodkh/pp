...
def event_type_detail(request, slug):
    ...
    if request.method == 'POST':
        ...
        if not active_event:
            return render(request, 'event_detail.html', {'event_type': event_type, 'partners': partners, 'achievements': achievements})
        form = RegistrationForm()
   
    return render(request, 'event_detail.html', {'event_type': event_type, 'event':active_event, 'form': form, 'partners': partners, 'achievements': achievements})
